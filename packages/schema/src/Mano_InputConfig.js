...
const Mano_InputConfig = (function () {
  ...
  function TestFileNameValid(officialFileName) {
    ...
    if (officialFileName === currentFileName) {
      return;
    }
    const message =
      `Do not rename the plugin file.<br>` +
  ...
  class EscapeSymbol extends I_SymbolDefine {
    ...
    static create(objText) {
      if (!objText) {
        const name = new MultiLanguageText(
          "menu/cancel",
          "メニュー/キャンセル"
        );
  ...
  class ExtendsSymbol extends I_SymbolDefine {
    ...
    debugInfo() {
      return `ot:${this.overwriteType()},id:${
        this._buttonId
      },keys:${this.getKeys()}`;
    }
  }
  ...
  class UnknowSymbol extends I_SymbolDefine {
    ...
    helpText() {
      return (
        setting.errorText.unknowSymbol.currentName() + "\n" + this.debugInfo()
      );
    }
  }
  ...
  class SymbolManager_T {
    ...
    *iterateSymbolList() {
     ...
     */
    allMandatorySymbols() {
      return this.getSymbolList().filter(function (def) {
        return def.isMandatory();
      });
    }
  ...
  setupExtendsSymbols(
    JSON.parse(getParam().extendsMapper || "[]"),
    symbolManager,
    ButtonManager,
    function (arg) {
  ...
  class DeviceLayout extends I_DeviceLayout {
    ...
    helpText() {
      const base = super.helpText();
      const text = `${this.deviceName()}\n${base}`;
      return text;
    }
  ...
  function createText(params) {
    const guid = new MultiLanguageText(
      "This is an unknown symbol. Add an item to the input extension",
      "不明なシンボルです 入力拡張に項目を追加してください"
    );

    return {
      gamepadConfigCommandText: MultiLanguageText.create(
        params.gamepadConfigCommandText
      ),
      keyConfigCommandText: MultiLanguageText.create(
        params.keyConfigCommandText
      ),
      mapperDelete: MultiLanguageText.create(params.mapperDelete),
      //TODO:安定したら今後のバージョンで消す
      gamepadIsNotConnected: MultiLanguageText.create(
        params.GamepadIsNotConnectedText
      ),
  ...
  function createErrorTexts() {
    const advanceSymbolInvalid = new MultiLanguageText(
      "",
      "拡張シンボルは標準シンボルと異なる内容でなければいけません。"
    );
    const initFauled = new MultiLanguageText(
      "The initialization process was not performed correctly. \nThere is a possibility of a plugin conflict. \nMove the plugin down may help.",
      "初期化処理が正しく行われませんでした。\nプラグインの競合の可能性があります。\nプラグインを下の方に移動すると解決する場合があります。"
    );
    const unknowSymbol = new MultiLanguageText(
      "This is an unknown symbol. Add an item to the input extension",
      "不明なシンボルです 入力拡張に項目を追加してください"
    );
    const symbolEmpty = new MultiLanguageText(
      "The symbol is not set \n Check the contents of the inputExtension from the plugin parameters",
      "シンボルが設定されていません\nプラグインパラメータから拡張設定の内容を確認してください"
    );
    const nameEmpty = new MultiLanguageText(
      "The name for display is not set\nsymbol:",
      "表示用の名称が設定されていません\nsymbol:"
    );
  ...
  class DeviceXXX {
    ...
    setupDefaultMapper() {
      ...
      this._defaultGamepadMapper = new InputDevice_Readonly(
        Object.freeze(objectClone(Input.gamepadMapper))
      );
    }
    // keyMapper(){
  ...
  function createGamepad() {
    //TODO:helpText差し込み
    const nintendo = createGamepadLayout(
      "N",
      "Nintendo Like",
      "B",
      "A",
    ...
    const xbox = createGamepadLayout("X", "Xbox Like", "A", "B", "X", "Y");
    const playstation = createGamepadLayout(
      "P",
      "PlayStation Like A",
      "×",
      "○",
    ...
    const ps5DualDualSense = createGamepadLayout(
      "PS5_Windows",
      "PlayStation Like B",
      "□",
      "×",
    ...
    const gamepadLayoutSelector = new LayoutSelecter([
      numberGamepad,
      nintendo,
      xbox,
      playstation,
  ...
  class Key_CommandManager_T {
    ...
    createCommandList_ForGamepad() {
      ...
      return [apply, reset, layout, exit];
    }
  }
  /**
   *
  ...
  function createCommandV2(handlerName, name_mText, width, helpText_mText) {
    ...
    const help = helpText_mText
      ? MultiLanguageText.create(helpText_mText)
      : null;
  ...
  function createCommandManager() {
    const params = getParam();
    const save = createCommandV2(
      "apply",
      params.saveCommand,
      params.saveCommandWidth,
      params.saveDescription
    );
    const wasd = createCommandV2(
      "WASD",
      params.WASDCommand,
      params.WASDwidth,
      params.WASDhelp
    );
    const exit = createCommandV2(
      "EXIT",
      params.exitCommand,
      params.exitWidth,
      params.exitHelp
    );
    const reset = createCommandV2(
      "reset",
      params.resetCommand,
      params.resetWidth,
      params.resetDescription
    );
    const changeLayout = createButtonLayoutChangeCommand();
    const changeKeyLayout = createCommandV2(
      "KeyLayout",
      params.changeKeyLayoutCommand,
      params.changeKeyLayoutCommandWidth,
      null
    );
    ...
    const comandSet = {
      ...
      wasd,
      reset,
      keyLayout: changeKeyLayout,
      buttonLayout: changeLayout,
    };
    return comandSet;
  ...
  class MV_Impriment extends I_MVMZ_Workaround {
    ...
    mainAreaHeigth() {
      const helpAreaHeight = this.calcWindowHeight(
        this.helpWindowLines(),
        false
      );
  ...
  class InputConfigManager_T {
    ...
    validateSaveData(config) {
      return {
        ...
        keyMapper:
          (config && config.keyMapper) || this._defaultKeyborad.cloneMapper(),
        gamepadMapper:
          (config && config.gamepadMapper) ||
          this._defaultGamepad.cloneMapper(),
  ...
  const InputConfigManager = (function () {
    const mvmz =
      Utils.RPGMAKER_NAME === "MV" ? new MV_Impriment() : new MZ_Impriment();
    const readonlyData = new InputConfigReadOnly(mvmz);

    return new InputConfigManager_T(
      readonlyData,
      setting.gamepad,
      setting.Keyboard
    );
  ...
  class V8_Item_LayoutCommand extends V8Item_Command {
    /**
     * @param {CommandConcept} command
     */
    constructor(command) {
  ...
  class Window_GamepadConfig_V8 extends Window_Selectable_InputConfigVer {
    ...
    initialize(rect) {
      ...
      this._layoutCommand = new V8_Item_LayoutCommand(
        setting.command.buttonLayout
      );
      this._exitCommand = new V8Item_Command(CommandManager.exit);
      this._resetCommand = new V8Item_Command(CommandManager.reset);
      this._applyCommand = new V8_Item_ApplyCommand(
        CommandManager.save,
        this._tmpMapper
      );
  ...
  class Scene_GamepadConfig_V8 extends Scene_MenuBaseMVMZ {
    ...
    symbolListHeight() {
      const mainAreaHeight =
        InputConfigManager.getWorkaround().mainAreaHeigth(this);
    ...
    createGamepadWindow() {
      ...
      ww.setHandler(
        setting.command.exit.handle(),
        this.onGamepadCancel.bind(this)
      );
      ...
      ww.setHandler(
        setting.command.buttonLayout.handle(),
        this.onChangeLayoutOk.bind(this)
      );
      this.addWindow(ww);
  ...
  class Window_KeyConfig_MA_V10 extends Window_WideButton_Selectable {
    ...
    drawKey(keyName, symbolName, rect) {
      ...
      this.drawText(
        symbolName,
        rect.x,
        rect.y + this.symbolNameFontSize() + 2,
        rect.width,
        "center"
      );
    }
    ...
    drawItem(index) {
      ...
      const rect =
        item.isCommand() || item === KEYS.ENTER_JIS
          ? this.itemLineRect(index)
          : this.itemRectWithPadding(index);
    ...
    itemHeight() {
      return (
        this.keyNameFontSize() +
        this.symbolNameFontSize() +
        this.rowSpacing() +
        2
      );
    }
    ...
    layoutHelpText() {
      return this._layout.deviceName();
    }
    /**
  ...
  class Scene_KeyConfig_V10 extends Scene_MenuBaseMVMZ {
    ...
    createKeyboardWindow() {
      ...
      kw.setHandler(
        setting.command.reset.handle(),
        this.onKeyboardReset.bind(this)
      );
      kw.setHandler(
        setting.command.exit.handle(),
        this.onKeyboardCancel.bind(this)
      );
      kw.setHandler(
        setting.command.keyLayout.handle(),
        this.onKeyLayout.bind(this)
      );
  ...
  function setupPP_option() {
    ...
    if (PP_Option && PP_Option.Manager) {
      PP_Option.Manager.addOptionEX(
        MA_GAMEPAD_CONFIG,
        currentGamepadConfigText,
        function (w, s, i) {
          Mano_InputConfig.gotoGamepad();
        }
      );
  ...
  if (Utils.RPGMAKER_NAME == "MV") {
    (function () {
      Window_Selectable_InputConfigVer.prototype.smoothSelect = function (
        index
      ) {
        this.select(index);
      };
      ...
      //     setupDefaultMapper();
      // };
      Window_Selectable_InputConfigVer.prototype.drawItemBackground =
        function () {};
      ...
      Window_Selectable_InputConfigVer.prototype.itemRectWithPadding =
        Window_Selectable_InputConfigVer.prototype.itemRectForText;
    })();
  } else {
